...
``` usage: chair release APP patch|minor|major --from-branch staging ```
...
```usage: chair prepare-staging APP```
...
- Deploy staging branch on vmraid.io, erpadda.org, vmraid.erpadda.com. 
...
- Use release command to create release,
``` usage: chair release APP patch --from-branch hotfix ```
