...
tmp_chair_repo = os.path.join('/', 'tmp', '.chair')
...
def check_environment():
	...
	if message:
		log("Chair's CLI needs these to be defined!", level=3)
...
def check_system_package_managers():
	if 'Darwin' in os.uname():
		if not shutil.which('brew'):
			raise Exception('''
			Please install brew package manager before proceeding with chair setup. Please run following
...
def import_with_install(package):
	# copied from https://discuss.erpadda.com/u/nikunj_patel
	# https://discuss.erpadda.com/t/easy-install-setup-guide-for-erpadda-installation-on-ubuntu-20-04-lts-with-some-modification-of-course/62375/5
...
def install_prerequisites():
	# pre-requisites for chair repo cloning
	run_os_command({
		'apt-get': [
			'sudo apt-get update',
...
def install_chair(args):
	# clone chair repo
	if not args.run_travis:
		clone_chair_repo(args)
	...
	extra_vars.update(vmraid_user=args.user)

	extra_vars.update(user_directory=get_user_home_directory(args.user))

	if os.path.exists(tmp_chair_repo):
		repo_path = tmp_chair_repo
	else:
		repo_path = os.path.join(os.path.expanduser('~'), 'chair')
	...
	if args.version <= 10:
		vmraid_branch = "{0}.x.x".format(args.version)
		erpadda_branch = "{0}.x.x".format(args.version)
	else:
		vmraid_branch = "version-{0}".format(args.version)
		erpadda_branch = "version-{0}".format(args.version)

	# Allow override of vmraid_branch and erpadda_branch, regardless of args.version (which always has a default set)
	if args.vmraid_branch:
		vmraid_branch = args.vmraid_branch
	if args.erpadda_branch:
		erpadda_branch = args.erpadda_branch

	extra_vars.update(vmraid_branch=vmraid_branch)
	extra_vars.update(erpadda_branch=erpadda_branch)

	chair_name = 'vmraid-chair' if not args.chair_name else args.chair_name
	extra_vars.update(chair_name=chair_name)

	# Will install ERPAdda production setup by default
	if args.without_erpadda:
		log("Initializing chair {chair_name}:\n\tVMRaid Branch: {vmraid_branch}\n\tERPAdda will not be installed due to --without-erpadda".format(chair_name=chair_name, vmraid_branch=vmraid_branch))
	else:
		log("Initializing chair {chair_name}:\n\tVMRaid Branch: {vmraid_branch}\n\tERPAdda Branch: {erpadda_branch}".format(chair_name=chair_name, vmraid_branch=vmraid_branch, erpadda_branch=erpadda_branch))
	run_playbook('site.yml', sudo=True, extra_vars=extra_vars)

	if os.path.exists(tmp_chair_repo):
		shutil.rmtree(tmp_chair_repo)


def clone_chair_repo(args):
	'''Clones the chair repository in the user folder'''
	branch = args.chair_branch or 'develop'
	repo_url = args.repo_url or 'https://github.com/vmraid/chair'

	if os.path.exists(tmp_chair_repo):
		log('Not cloning already existing Chair repository at {tmp_chair_repo}'.format(tmp_chair_repo=tmp_chair_repo))
		return 0
	elif args.without_chair_setup:
		clone_path = os.path.join(os.path.expanduser('~'), 'chair')
		log('--without-chair-setup specified, clone path is: {clone_path}'.format(clone_path=clone_path))
	else:
		clone_path = tmp_chair_repo
		# Not logging repo_url to avoid accidental credential leak in case credential is embedded in URL
		log('Cloning chair repository branch {branch} into {clone_path}'.format(branch=branch, clone_path=clone_path))

	success = run_os_command(
		{'git': 'git clone --quiet {repo_url} {chair_repo} --depth 1 --branch {branch}'.format(
			repo_url=repo_url, chair_repo=clone_path, branch=branch)}
	)
...
def get_passwords(args):
	...
	and creates passwords.txt in the chair user's home directory
	"""
	log("Input MySQL and VMRaid Administrator passwords:")
	ignore_prompt = args.run_travis or args.without_chair_setup
...
def run_playbook(playbook_name, sudo=False, extra_vars=None):
	...
	if os.path.exists(tmp_chair_repo):
		...
		cwd = os.path.join(os.path.expanduser('~'), 'chair')

	playbooks_locations = [os.path.join(cwd, 'chair', 'playbooks'), os.path.join(cwd, 'playbooks')]
...
def parse_commandline_args():
	...
	parser = argparse.ArgumentParser(description='VMRaid Installer')
	...
	args_group.add_argument('--production', dest='production', action='store_true', default=False, help='Setup Production environment for chair')
	parser.add_argument('--site', dest='site', action='store', default='site1.local', help='Specify name for your first ERPAdda site')
	...
	parser.add_argument('--user', dest='user', help='Install vmraid-chair for this user')
	parser.add_argument('--chair-branch', dest='chair_branch', help='Clone a particular branch of chair repository')
	parser.add_argument('--repo-url', dest='repo_url', help='Clone chair from the given url')
	parser.add_argument('--vmraid-repo-url', dest='vmraid_repo_url', action='store', default='https://github.com/vmraid/vmraid', help='Clone vmraid from the given url')
	parser.add_argument('--vmraid-branch', dest='vmraid_branch', action='store', help='Clone a particular branch of vmraid')
	parser.add_argument('--erpadda-repo-url', dest='erpadda_repo_url', action='store', default='https://github.com/vmraid/erpadda', help='Clone erpadda from the given url')
	parser.add_argument('--erpadda-branch', dest='erpadda_branch', action='store', help='Clone a particular branch of erpadda')
	parser.add_argument('--without-erpadda', dest='without_erpadda', action='store_true', default=False, help='Prevent fetching ERPAdda')
	# direct provision to install versions
	parser.add_argument('--version', dest='version', action='store', default=13, type=int, help='Clone particular version of vmraid and erpadda')
	...
	parser.add_argument('--without-chair-setup', dest='without_chair_setup', action='store_true', default=False, help=argparse.SUPPRESS)
	# whether to overwrite an existing chair
	parser.add_argument('--overwrite', dest='overwrite', action='store_true', default=False, help='Whether to overwrite an existing chair')
	# set passwords
	...
	parser.add_argument('--chair-name', dest='chair_name', help='Create chair with specified name. Default name is vmraid-chair')
...
if __name__ == '__main__':
	...
	log("Chair + VMRaid + ERPAdda has been successfully installed!")
