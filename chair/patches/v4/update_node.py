...
def execute(chair_path):
	...
	if node_exec:
		...
		Please install latest node version before running "chair update". For installation instructions
	...
	if node_ver < expected_node_ver:
		click.echo('''
		Please update node to latest version before running "chair update".
		Please install latest node version before running "chair update". For installation instructions
		please refer "Debian and Ubuntu based Linux distributions" section or "Enterprise Linux and
		Fedora" section depending upon your OS on the following link,
		"https://nodejs.org/en/download/package-manager/"
