...
from chair.utils.chair import post_upgrade, patch_sites, build_assets


@click.command('update', help="Performs an update operation on current chair. Without any flags will backup, pull, setup requirements, build, run patches and restart chair. Using specific flags will only do certain tasks instead of all")
@click.option('--pull', is_flag=True, help="Pull updates for all the apps in chair")
@click.option('--apps', type=str)
@click.option('--patch', is_flag=True, help="Run migrations for all sites in the chair")
@click.option('--build', is_flag=True, help="Build JS and CSS assets for the chair")
@click.option('--requirements', is_flag=True, help="Update requirements. If run alone, equivalent to `chair setup requirements`")
...
def update(pull, apps, patch, build, requirements, restart_supervisor, restart_systemd, no_backup, no_compile, force, reset):
	from chair.utils.chair import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from chair.utils.app import switch_to_branch
	switch_to_branch(branch=branch, apps=list(apps), upgrade=upgrade)


@click.command('switch-to-develop')
def switch_to_develop(upgrade=False):
	"Switch vmraid and erpadda to develop branch"
	from chair.utils.app import switch_to_develop
	switch_to_develop(apps=['vmraid', 'erpadda'])
