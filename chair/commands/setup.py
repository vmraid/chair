...
from chair.utils import exec_cmd, run_playbook, which


@click.group(help="Setup command group for enabling setting up a VMRaid environment")
def setup():
	pass
...
def setup_sudoers(user):
	from chair.utils.system import setup_sudoers
	setup_sudoers(user)
...
def setup_nginx(yes=False):
	...
	make_nginx_conf(chair_path=".", yes=yes)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from chair.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False):
	...
	from chair.config.supervisor import update_supervisord_config, generate_supervisor_config
	...
	generate_supervisor_config(chair_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add VMRaid fonts to system")
def setup_fonts():
	from chair.utils.system import setup_fonts
	setup_fonts()


@click.command("production", help="Setup VMRaid production environment for specific user")
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from chair.config.production_setup import setup_production
	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for chair backups")
def setup_backups():
	from chair.chair import Chair
	Chair(".").setup.backups()


@click.command("env", help="Setup virtualenv for chair")
@click.option("--python", type = str, default = "python3", help = "Path to Python Executable.")
def setup_env(python="python3"):
	from chair.chair import Chair
	return Chair(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/chair/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/chair/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from chair.config.lets_encrypt import setup_letsencrypt
	setup_letsencrypt(site, custom_domain, chair_path=".", interactive=not non_interactive)


@click.command("wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant chair")
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from chair.config.lets_encrypt import setup_wildcard_ssl
	setup_wildcard_ssl(domain, email, chair_path=".", exclude_base_domain=exclude_base_domain)


@click.command("procfile", help="Generate Procfile for chair start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		chair.setup.requirements(apps=apps)

	elif not node and not dev:
		chair.setup.python(apps=apps)

	elif not python and not dev:
		chair.setup.node(apps=apps)

	else:
		from chair.utils.chair import install_python_dev_dependencies
...
@click.command("manager", help="Setup chair-manager.local site with the chair_manager app installed on it")
@click.option("--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True)
@click.option("--port", help="Port on which you want to run chair manager", default=23624)
@click.option("--domain", help="Domain on which you want to run chair manager")
def setup_manager(yes=False, port=23624, domain=None):
	from chair.chair import Chair
	from chair.config.nginx import make_chair_manager_nginx_conf

	create_new_site = True

	if "chair-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("chair new-site --force chair-manager.local")

	if "chair_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("chair get-app chair_manager")

	exec_cmd("chair --site chair-manager.local install-app chair_manager")

	chair_path = "."
	chair = Chair(chair_path)

	if chair.conf.get("restart_supervisor_on_update") or chair.conf.get("restart_systemd_on_update"):
		# implicates a production setup or so I presume
		if not domain:
			print("Please specify the site name on which you want to host chair-manager using the 'domain' flag")
			sys.exit(1)

		if domain not in chair.sites:
			raise Exception("No such site")

		make_chair_manager_nginx_conf(chair_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from chair.config.site_config import add_domain
	add_domain(site, domain, ssl_certificate, ssl_certificate_key, chair_path=".")
...
def remove_domain(domain, site=None):
	...
	from chair.config.site_config import remove_domain
	remove_domain(site, domain, chair_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from chair.config.site_config import sync_domains
	changed = sync_domains(site, domains, chair_path=".")
...
@click.option("--stop", help="Stop chair services", is_flag=True, default=False)
...
def setup_systemd(user=None, yes=False, stop=False, create_symlinks=False, delete_symlinks=False):
	from chair.config.systemd import generate_systemd_config
	generate_systemd_config(chair_path=".", user=user, yes=yes,
