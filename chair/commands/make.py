...
@click.command("init", help="Initialize a new chair instance in the specified path")
@click.argument("path")
@click.option(
	"--version",
	"--vmraid-branch",
	"vmraid_branch",
	default=None,
	help="Clone a particular branch of vmraid",
)
@click.option(
	"--ignore-exist", is_flag=True, default=False, help="Ignore if Chair instance exists."
)
@click.option(
...
@click.option("--vmraid-path", default=None, help="path to vmraid repo")
...
@click.option(
	"--no-backups",
	is_flag=True,
	help="Do not set up automatic periodic backups for all sites on this chair",
)
@click.option(
...
def init(
	...
	from chair.utils import log
	from chair.utils.system import init

	if not ignore_exist and os.path.exists(path):
		log(f"Chair instance already exists at {path}", level=2)
		return
	...
	try:
		...
		log(f"Chair {path} initialized", level=1)
	except SystemExit:
		raise
	except Exception:
		import shutil
		...
		from chair.utils import get_traceback
...
def drop(path):
	...
	from chair.exceptions import ChairNotFoundError, ValidationError

	chair = Chair(path)

	if not chair.exists:
		raise ChairNotFoundError(f"Chair {chair.name} does not exist")

	if chair.sites:
		raise ValidationError("Cannot remove non-empty chair directory")
...
@click.command(
	["get", "get-app"],
	help="Clone an app from the internet or filesystem and set it up in your chair",
)
...
@click.option("--skip-assets", is_flag=True, default=False, help="Do not build assets")
@click.option(
	"--init-chair", is_flag=True, default=False, help="Initialize Chair if not in one"
)
@click.option(
	"--resolve-deps",
	is_flag=True,
...
def get_app(
	...
	"clone an app from the internet and set it up in your chair"
	from chair.app import get_app
	...
	get_app(
		...
		skip_assets=skip_assets,
		overwrite=overwrite,
		init_chair=init_chair,
		resolve_deps=resolve_deps,
	)

@click.command("new-app", help="Create a new VMRaid application under apps folder")
@click.option(
	"--no-git",
	is_flag=True,
	flag_value="--no-git",
	help="Do not initialize git repository for the app (available in VMRaid v14+)"
)
@click.argument("app-name")
def new_app(app_name, no_git=None):
	from chair.app import new_app
...
@click.command(
	...
	help=(
		"Completely remove app from chair and re-build assets if not installed on any site"
	),
)
@click.argument("app-name")
...
def include_app_for_update(app_name):
	...
	from chair.app import remove_from_excluded_apps_txt
...
@click.command(
	...
	help="For pip help use `chair pip help [COMMAND]` or `chair pip [COMMAND] -h`",
)
@click.argument("args", nargs=-1)
@click.pass_context
def pip(ctx, args):
	"Run pip commands in chair env"
	import os
