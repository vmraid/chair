...
from chair.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help='Change chair configuration')
def config():
	pass
...
@click.command('dns_multitenant', help='Enable/Disable chair multitenancy on running chair update')
@click.argument('state', type=click.Choice(['on', 'off']))
...
def set_common_config(configs):
	...
	update_config(common_site_config, chair_path='.')


@click.command('remove-common-config', help='Remove specific keys from current chair\'s common config')
@click.argument('keys', nargs=-1)
def remove_common_config(keys):
	from chair.chair import Chair
	common_site_config = Chair('.').conf
	for key in keys:
