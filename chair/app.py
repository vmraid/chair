# imports - standard imports
import functools
import json
import logging
import os
import re
import shutil
import subprocess
import sys
import typing
from collections import OrderedDict
from datetime import date
from urllib.parse import urlparse

# imports - third party imports
import click

# imports - module imports
import chair
from chair.exceptions import NotInChairDirectoryError
from chair.utils import (
	fetch_details_from_tag,
	get_available_folder_name,
	is_chair_directory,
	is_git_url,
	is_valid_vmraid_branch,
	log,
	run_vmraid_cmd,
)
from chair.utils.chair import (
	build_assets,
	install_python_dev_dependencies,
)
from chair.utils.render import step

if typing.TYPE_CHECKING:
	from chair.chair import Chair


logger = logging.getLogger(chair.PROJECT_NAME)


class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		"""
		name (str): This could look something like
			1. https://github.com/vmraid/healthcare.git
			2. git@github.com:vmraid/healthcare.git
			3. vmraid/healthcare@develop
			4. healthcare
			5. healthcare@develop, healthcare@v13.12.1

		References for Version Identifiers:
		 * https://www.python.org/dev/peps/pep-0440/#version-specifiers
		 * https://docs.npmjs.com/about-semantic-versioning

		class Healthcare(AppConfig):
			dependencies = [{"vmraid/erpadda": "~13.17.0"}]
		"""
		self.name = name.rstrip("/")
		self.remote_server = "github.com"
		self.to_clone = to_clone
		self.on_disk = False
		self.use_ssh = False
		self.from_apps = False
		self.is_url = False
		self.branch = branch
		self.mount_path = os.path.abspath(
			os.path.join(urlparse(self.name).netloc, urlparse(self.name).path)
		)
		self.setup_details()

	def setup_details(self):
		# fetch meta from installed apps
		if (
			not self.to_clone
			and hasattr(self, "chair")
			and os.path.exists(os.path.join(self.chair.name, "apps", self.name))
		):
			self.from_apps = True
			self._setup_details_from_installed_apps()

		# fetch meta for repo on mounted disk
		elif os.path.exists(self.mount_path):
			self.on_disk = True
			self._setup_details_from_mounted_disk()

		# fetch meta for repo from remote git server - traditional get-app url
		elif is_git_url(self.name):
			self.is_url = True
			if self.name.startswith("git@") or self.name.startswith("ssh://"):
				self.use_ssh = True
			self._setup_details_from_git_url()

		# fetch meta from new styled name tags & first party apps on github
		else:
			self._setup_details_from_name_tag()

	def _setup_details_from_mounted_disk(self):
		self.org, self.repo, self.tag = os.path.split(self.mount_path)[-2:] + (self.branch,)

	def _setup_details_from_name_tag(self):
		self.org, self.repo, self.tag = fetch_details_from_tag(self.name)
		self.tag = self.tag or self.branch

	def _setup_details_from_installed_apps(self):
		self.org, self.repo, self.tag = os.path.split(
			os.path.join(self.chair.name, "apps", self.name)
		)[-2:] + (self.branch,)

	def _setup_details_from_git_url(self):
		return self.__setup_details_from_git()

	def __setup_details_from_git(self):
		if self.use_ssh:
			_first_part, _second_part = self.name.split(":")
			self.remote_server = _first_part.split("@")[-1]
			self.org, _repo = _second_part.rsplit("/", 1)
		else:
			self.remote_server, self.org, _repo = self.name.rsplit("/", 2)

		self.tag = self.branch
		self.repo = _repo.split(".")[0]

	@property
	def url(self):
		if self.from_apps:
			return os.path.abspath(os.path.join("apps", self.name))

		if self.on_disk:
			return self.mount_path

		if self.is_url:
			return self.name

		if self.use_ssh:
			return self.get_ssh_url()

		return self.get_http_url()

	def get_http_url(self):
		return f"https://{self.remote_server}/{self.org}/{self.repo}.git"

	def get_ssh_url(self):
		return f"git@{self.remote_server}:{self.org}/{self.repo}.git"


@functools.lru_cache(maxsize=None)
class App(AppMeta):
	def __init__(self, name: str, branch: str = None, chair: "Chair" = None, *args, **kwargs):
		self.chair = chair
		self.required_by = None
		self.local_resolution = []
		super().__init__(name, branch, *args, **kwargs)

	@step(title="Fetching App {repo}", success="App {repo} Fetched")
	def get(self):
		branch = f"--branch {self.tag}" if self.tag else ""
		shallow = "--depth 1" if self.chair.shallow_clone else ""

		fetch_txt = f"Getting {self.repo}"
		click.secho(fetch_txt, fg="yellow")
		logger.log(fetch_txt)

		self.chair.run(
			f"git clone {self.url} {branch} {shallow} --origin upstream",
			cwd=os.path.join(self.chair.name, "apps"),
		)

	@step(title="Archiving App {repo}", success="App {repo} Archived")
	def remove(self):
		active_app_path = os.path.join("apps", self.repo)
		archived_path = os.path.join("archived", "apps")
		archived_name = get_available_folder_name(f"{self.repo}-{date.today()}", archived_path)
		archived_app_path = os.path.join(archived_path, archived_name)
		log(f"App moved from {active_app_path} to {archived_app_path}")
		shutil.move(active_app_path, archived_app_path)

	@step(title="Installing App {repo}", success="App {repo} Installed")
	def install(
		self,
		skip_assets=False,
		verbose=False,
		resolved=False,
		restart_chair=True,
		ignore_resolution=False,
	):
		import chair.cli
		from chair.utils.app import get_app_name

		verbose = chair.cli.verbose or verbose
		app_name = get_app_name(self.chair.name, self.repo)
		if not resolved and self.repo != "vmraid" and not ignore_resolution:
			click.secho(
				f"Ignoring dependencies of {self.name}. To install dependencies use --resolve-deps",
				fg="yellow",
			)

		install_app(
			app=app_name,
			tag=self.tag,
			chair_path=self.chair.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_chair=restart_chair,
			resolution=self.local_resolution
		)

	@step(title="Cloning and installing {repo}", success="App {repo} Installed")
	def install_resolved_apps(self, *args, **kwargs):
		self.get()
		self.install(*args, **kwargs, resolved=True)

	@step(title="Uninstalling App {repo}", success="App {repo} Uninstalled")
	def uninstall(self):
		self.chair.run(f"{self.chair.python} -m pip uninstall -y {self.repo}")

	def _get_dependencies(self):
		from chair.utils.app import get_required_deps, required_apps_from_hooks

		if self.on_disk:
			required_deps = os.path.join(self.mount_path, self.repo,'hooks.py')
			try:
				return required_apps_from_hooks(required_deps, local=True)
			except IndexError:
				return []
		try:
			required_deps = get_required_deps(self.org, self.repo, self.tag or self.branch)
			return required_apps_from_hooks(required_deps)
		except Exception:
			return []

	def update_app_state(self):
		from chair.chair import Chair
		chair = Chair(self.chair.name)
		chair.apps.sync(self.name, self.tag, self.local_resolution)


def make_resolution_plan(app: App, chair: "Chair"):
	"""
	decide what apps and versions to install and in what order
	"""
	resolution = OrderedDict()
	resolution[app.repo] = app

	for app_name in app._get_dependencies():
		dep_app = App(app_name, chair=chair)
		is_valid_vmraid_branch(dep_app.url, dep_app.branch)
		dep_app.required_by = app.name
		if dep_app.repo in resolution:
			click.secho(f"{dep_app.repo} is already resolved skipping", fg="yellow")
			continue
		resolution[dep_app.repo] = dep_app
		resolution.update(make_resolution_plan(dep_app, chair))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def add_to_appstxt(app, chair_path="."):
	from chair.chair import Chair

	apps = Chair(chair_path).apps

	if app not in apps:
		apps.append(app)
		return write_appstxt(apps, chair_path=chair_path)


def remove_from_appstxt(app, chair_path="."):
	from chair.chair import Chair

	apps = Chair(chair_path).apps

	if app in apps:
		apps.remove(app)
		return write_appstxt(apps, chair_path=chair_path)


def write_appstxt(apps, chair_path="."):
	with open(os.path.join(chair_path, "sites", "apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def get_excluded_apps(chair_path="."):
	try:
		with open(os.path.join(chair_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except IOError:
		return []


def add_to_excluded_apps_txt(app, chair_path="."):
	if app == "vmraid":
		raise ValueError("VMRaid app cannot be excludeed from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(chair_path=chair_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, chair_path=chair_path)


def write_excluded_apps_txt(apps, chair_path="."):
	with open(os.path.join(chair_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, chair_path="."):
	apps = get_excluded_apps(chair_path=chair_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, chair_path=chair_path)


def get_app(
	git_url,
	branch=None,
	chair_path=".",
	skip_assets=False,
	verbose=False,
	overwrite=False,
	init_chair=False,
	resolve_deps=False,
):
	"""chair get-app clones a VMRaid App from remote (GitHub or any other git server),
	and installs it on the current chair. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the chair_path is not a chair directory, a new chair is created named using the
	git_url parameter.
	"""
	import chair as _chair
	import chair.cli as chair_cli
	from chair.chair import Chair
	from chair.utils.app import check_existing_dir

	chair = Chair(chair_path)
	app = App(git_url, branch=branch, chair=chair)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	chair_setup = False
	restart_chair = not init_chair
	vmraid_path, vmraid_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, chair)
		click.secho("Following apps will be installed", fg="bright_blue")
		for idx, app in enumerate(reversed(resolution.values()), start=1):
			print(f"{idx}. {app.name} {f'(required by {app.required_by})' if app.required_by else ''}")

		if "vmraid" in resolution:
			# Todo: Make vmraid a terminal dependency for all vmraid apps.
			vmraid_path, vmraid_branch = resolution["vmraid"].url, resolution["vmraid"].tag

	if not is_chair_directory(chair_path):
		if not init_chair:
			raise NotInChairDirectoryError(
				f"{os.path.realpath(chair_path)} is not a valid chair directory. "
				"Run with --init-chair if you'd like to create a Chair too."
			)

		from chair.utils.system import init

		chair_path = get_available_folder_name(f"{app.repo}-chair", chair_path)
		init(
			path=chair_path,
			vmraid_path=vmraid_path,
			vmraid_branch=vmraid_branch if vmraid_branch else branch,
		)
		os.chdir(chair_path)
		chair_setup = True

	if chair_setup and chair_cli.from_command_line and chair_cli.dynamic_feed:
		_chair.LOG_BUFFER.append(
			{
				"message": f"Fetching App {repo_name}",
				"prefix": click.style("⏼", fg="bright_yellow"),
				"is_parent": True,
				"color": None,
			}
		)

	if resolve_deps:
		install_resolved_deps(
			chair,
			resolution,
			chair_path=chair_path,
			skip_assets=skip_assets,
			verbose=verbose,
		)
		return

	dir_already_exists, cloned_path = check_existing_dir(chair_path, repo_name)
	to_clone = not dir_already_exists

	# application directory already exists
	# prompt user to overwrite it
	if dir_already_exists and (
		overwrite
		or click.confirm(
			f"A directory for the application '{repo_name}' already exists. "
			"Do you want to continue and overwrite it?"
		)
	):
		shutil.rmtree(cloned_path)
		to_clone = True

	if to_clone:
		app.get()

	if (
		to_clone
		or overwrite
		or click.confirm("Do you want to reinstall the existing application?")
	):
		app.install(verbose=verbose, skip_assets=skip_assets, restart_chair=restart_chair)


def install_resolved_deps(
	chair,
	resolution,
	chair_path=".",
	skip_assets=False,
	verbose=False,
):
	from chair.utils.app import check_existing_dir

	if "vmraid" in resolution:
		# Terminal dependency
		del resolution["vmraid"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(chair_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = chair.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
					subprocess.
					check_output("git rev-parse --abbrev-ref HEAD", shell=True, cwd=path_to_app)
					.decode("utf-8")
					.rstrip()
					)
			try:
				if app.tag is None:
					current_remote = (
						subprocess.check_output(f"git config branch.{installed_branch}.remote", shell=True, cwd=path_to_app)
						.decode("utf-8")
						.rstrip()
					)

					default_branch = (
						subprocess.check_output(
							f"git symbolic-ref refs/remotes/{current_remote}/HEAD", shell=True, cwd=path_to_app
						)
						.decode("utf-8")
						.rsplit("/")[-1]
						.strip()
					)
					is_compatible = default_branch == installed_branch
				else:
					is_compatible = installed_branch == app.tag
			except Exception:
				is_compatible = False

			prefix = 'C' if is_compatible else 'Inc'
			click.secho(
				f"{prefix}ompatible version of {repo_name} is already installed",
				fg="green" if is_compatible else "red",
			)
			app.update_app_state()
			if click.confirm(
				f"Do you wish to clone and install the already installed {prefix}ompatible app"
			):
				click.secho(f"Removing installed app {app.name}", fg="yellow")
				shutil.rmtree(path_to_app)
			else:
				continue
		app.install_resolved_apps(skip_assets=skip_assets, verbose=verbose)


def new_app(app, no_git=None, chair_path="."):
	if chair.VMRAID_VERSION in (0, None):
		raise NotInChairDirectoryError(f"{os.path.realpath(chair_path)} is not a valid chair directory.")

	# For backwards compatibility
	app = app.lower().replace(" ", "_").replace("-", "_")
	apps = os.path.abspath(os.path.join(chair_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if chair.VMRAID_VERSION < 14:
			click.secho("VMRaid v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_vmraid_cmd(*args, chair_path=chair_path)
	install_app(app, chair_path=chair_path)


def install_app(
	app,
	tag=None,
	chair_path=".",
	verbose=False,
	no_cache=False,
	restart_chair=True,
	skip_assets=False,
	resolution=[]
):
	import chair.cli as chair_cli
	from chair.chair import Chair

	install_text = f"Installing {app}"
	click.secho(install_text, fg="yellow")
	logger.log(install_text)

	chair = Chair(chair_path)
	conf = chair.conf

	verbose = chair_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(chair_path, "apps", app))

	chair.run(f"{chair.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}")

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, chair_path=chair_path, verbose=verbose)

	if os.path.exists(os.path.join(app_path, "package.json")):
		chair.run("yarn install", cwd=app_path)

	chair.apps.sync(app, required=resolution, branch=tag)

	if not skip_assets:
		build_assets(chair_path=chair_path, app=app)

	if restart_chair:
		chair.reload()


def pull_apps(apps=None, chair_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from chair.chair import Chair
	from chair.utils.app import get_current_branch, get_remote

	chair = Chair(chair_path)
	rebase = "--rebase" if chair.conf.get("rebase_on_pull") else ""
	apps = apps or chair.apps
	excluded_apps = chair.excluded_apps

	# check for local changes
	if not reset:
		for app in apps:
			if app in excluded_apps:
				print(f"Skipping reset for app {app}")
				continue
			app_dir = get_repo_dir(app, chair_path=chair_path)
			if os.path.exists(os.path.join(app_dir, ".git")):
				out = subprocess.check_output("git status", shell=True, cwd=app_dir)
				out = out.decode("utf-8")
				if not re.search(r"nothing to commit, working (directory|tree) clean", out):
					print(
						f"""

Cannot proceed with update: You have local changes in app "{app}" that are not committed.

Here are your choices:

1. Merge the {app} app manually with "git pull" / "git pull --rebase" and fix conflicts.
1. Temporarily remove your changes with "git stash" or discard them completely
	with "chair update --reset" or for individual repositries "git reset --hard"
2. If your changes are helpful for others, send in a pull request via GitHub and
	wait for them to be merged in the core."""
					)
					sys.exit(1)

	for app in apps:
		if app in excluded_apps:
			print(f"Skipping pull for app {app}")
			continue
		app_dir = get_repo_dir(app, chair_path=chair_path)
		if os.path.exists(os.path.join(app_dir, ".git")):
			remote = get_remote(app)
			if not remote:
				# remote is False, i.e. remote doesn't exist, add the app to excluded_apps.txt
				add_to_excluded_apps_txt(app, chair_path=chair_path)
				print(
					f"Skipping pull for app {app}, since remote doesn't exist, and"
					" adding it to excluded apps"
				)
				continue

			if not chair.conf.get("shallow_clone") or not reset:
				is_shallow = os.path.exists(os.path.join(app_dir, ".git", "shallow"))
				if is_shallow:
					s = " to safely pull remote changes." if not reset else ""
					print(f"Unshallowing {app}{s}")
					chair.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, chair_path=chair_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if chair.conf.get("shallow_clone"):
					chair.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					chair.run(reset_cmd, cwd=app_dir)
					chair.run("git reflog expire --all", cwd=app_dir)
					chair.run("git gc --prune=all", cwd=app_dir)
				else:
					chair.run("git fetch --all", cwd=app_dir)
					chair.run(reset_cmd, cwd=app_dir)
			else:
				chair.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			chair.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(chair_path):
	chair_path = os.path.abspath(chair_path)
	celery_app = os.path.join(chair_path, "apps", "vmraid", "vmraid", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, chair_path="."):
	return os.path.join(chair_path, "apps", app)


def install_apps_from_path(path, chair_path="."):
	apps = get_apps_json(path)
	for app in apps:
		get_app(
			app["url"],
			branch=app.get("branch"),
			chair_path=chair_path,
			skip_assets=True,
		)


def get_apps_json(path):
	import requests

	if path.startswith("http"):
		r = requests.get(path)
		return r.json()

	with open(path) as f:
		return json.load(f)
