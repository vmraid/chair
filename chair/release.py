...
def release(chair_path, app, bump_type, from_branch, to_branch,
		remote='upstream', owner='vmraid', repo_name=None, frontport=True):

	confirm_testing()
	config = get_config(chair_path)

	if not config.get('release_chair'):
		print('chair not configured to release')
		sys.exit(1)
	...
	validate(chair_path, config)

	bump(chair_path, app, bump_type, from_branch=from_branch, to_branch=to_branch, owner=owner,
		repo_name=repo_name, remote=remote, frontport=frontport)

def validate(chair_path, config):
	import requests
...
def confirm_testing():
	...
	print('Manual Testing Checklisk: https://github.com/vmraid/chair/wiki/Testing-Checklist')
	print('')
	print('')
	click.confirm('Is manual testing done?', abort = True)
	click.confirm('Have you added the change log?', abort = True)

def bump(chair_path, app, bump_type, from_branch, to_branch, remote, owner, repo_name=None, frontport=True):
	assert bump_type in ['minor', 'major', 'patch', 'stable', 'prerelease']

	repo_path = os.path.join(chair_path, 'apps', app)
	push_branch_for_old_major_version(chair_path, bump_type, app, repo_path, from_branch, to_branch, remote, owner)
...
def create_github_release(repo_path, tag_name, message, remote='upstream', owner='vmraid', repo_name=None,
...
def push_branch_for_old_major_version(chair_path, bump_type, app, repo_path, from_branch, to_branch, remote, owner):
	if bump_type != 'major':
		return
