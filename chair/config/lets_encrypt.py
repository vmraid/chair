...
from chair.config.site_config import get_domains, remove_domain, update_site_config
from chair.chair import Chair
from chair.utils import exec_cmd
from chair.utils.chair import update_common_site_config
from chair.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, chair_path, interactive):

	site_path = os.path.join(chair_path, "sites", site, "site_config.json")
	...
	if not Chair(chair_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, chair_path, interactive)
	setup_crontab()


def create_config(site, custom_domain):
	config = chair.config.env().get_template('letsencrypt.cfg').render(domain=custom_domain or site)
...
def run_certbot_and_setup_ssl(site, custom_domain, chair_path, interactive=True):
	service('nginx', 'stop')
	get_certbot()
	...
	if custom_domain:
		remove_domain(site, custom_domain, chair_path)
		domains = get_domains(site, chair_path)
		ssl_config['domain'] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, { "domains": domains }, chair_path=chair_path)
	else:
		update_site_config(site, ssl_config, chair_path=chair_path)

	make_nginx_conf(chair_path)
	service('nginx', 'start')
...
def setup_wildcard_ssl(domain, email, chair_path, exclude_base_domain):
