...
from chair.config.supervisor import generate_supervisor_config, update_supervisord_config
from chair.config.systemd import generate_systemd_config
from chair.chair import Chair
from chair.utils import exec_cmd, which, get_chair_name, get_cmd_output, log
from chair.utils.system import fix_prod_setup_perms
from chair.exceptions import CommandFailedError
...
def setup_production_prerequisites():
	...
	if not which("fail2ban-client"):
		exec_cmd("chair setup role fail2ban")
	if not which("nginx"):
		exec_cmd("chair setup role nginx")
	if not which("supervisord"):
		exec_cmd("chair setup role supervisor")


def setup_production(user, chair_path='.', yes=False):
	...
	if conf.get('restart_systemd_on_update'):
		...
		generate_systemd_config(chair_path=chair_path, user=user, yes=yes)
	else:
		print("Setting Up supervisor...")
		update_supervisord_config(user=user, yes=yes)
		generate_supervisor_config(chair_path=chair_path, user=user, yes=yes)

	print("Setting Up NGINX...")
	make_nginx_conf(chair_path=chair_path, yes=yes)
	fix_prod_setup_perms(chair_path, vmraid_user=user)
	...
	if conf.get('restart_supervisor_on_update'):
		...
		supervisor_conf = os.path.join(get_supervisor_confdir(), f'{chair_name}.{supervisor_conf_extn}')

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(os.path.abspath(os.path.join(chair_path, 'config', 'supervisor.conf')), supervisor_conf)

	if not os.path.islink(nginx_conf):
		os.symlink(os.path.abspath(os.path.join(chair_path, 'config', 'nginx.conf')), nginx_conf)
...
def disable_production(chair_path='.'):
	chair_name = get_chair_name(chair_path)
	conf = Chair(chair_path).conf
...
def service(service_name, service_option):
	if os.path.basename(which('systemctl') or '') == 'systemctl' and is_running_systemd():
		...
		service_manager = os.environ.get("CHAIR_SERVICE_MANAGER")
		if service_manager:
			service_manager_command = (
				os.environ.get("CHAIR_SERVICE_MANAGER_COMMAND")
